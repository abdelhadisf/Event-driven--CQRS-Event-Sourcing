...
public class HotelProjection {
    ...
    public void addChambre(ChambreAddedEvent event){
        ...
        HotelAggregate hotelAggregate=hotelRepository.findById(event.getIdH()).get();

        chambreRepository.save(new ChambreAggregate(event.getIdCh(), event.getType(), event.getEtage(), hotelAggregate));
